...
#'
#'  The test is run for each Modality values and each Farm with a p-adjustement
#'  step.
#'
#' @param MiteMap (required) The result of import_mitemap ($resulting_data) for raw_data
#' @param modality The modality to separate test
...
#' @examples
#' binom_test_mitemap(HH_example$resulting_data)
#' binom_test_mitemap(HH_example$resulting_data, p.adjust_method = "bonferroni")
binom_test_mitemap <- function(MiteMap, modality = "Modality", p.adjust_method = "BH") {
  MiteMap_bin <- MiteMap |>
    ...
    dplyr::group_by(Farm, .data[[modality]]) |>
    dplyr::summarise(
      n = n(),
